...
@Component({
  selector: 'app-proveeduria-inventario',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="mb-4">
      <h2 class="mb-2 text-1100">Inventario General de Proveeduría</h2>
      <h5 class="text-700 fw-semi-bold">Gestión centralizada de suministros institucionales</h5>
    </div>
...
export class InventarioComponent implements OnInit {
  ...
  esOperativo(): boolean {
    return this.authSvc.hasRole('ADMINISTRADOR', 'PROVEEDURIA');
  }
  ...
  egresar(lote: LoteInventario): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.svc.egresarLote(lote.id_lote, 'proveeduria').subscribe(() => {
  ...
  editar(lote: LoteInventario): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed && result.value) {
        this.svc.editarLote(lote.id_lote, result.value, 'proveeduria').subscribe(() => {
